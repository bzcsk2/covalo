...
[loop.ts:586](../packages/core/src/loop.ts) 的映射逻辑把所有非 `"two-stage"` 值（包括默认的 `"auto"`）当作 `"direct"` 处理：
...
编辑 [packages/core/src/tool-routing/two-stage-router.ts](../packages/core/src/tool-routing/two-stage-router.ts)，把类型扩展为：
...
编辑 [loop.ts:586](../packages/core/src/loop.ts)，把 `"auto"` 走自动检测：
...
编辑 [loop.ts](../packages/core/src/loop.ts)，在工具调用处理逻辑中添加 `select_category` 拦截分支：
...
[cli/tui.ts:345](../packages/cli/src/tui.ts) 创建了一个新的 `QuestionService` 实例传给 `WorkflowCoordinator`。`runWaitingUser`（[coordinator.ts:718](../packages/core/src/workflow-coordinator/coordinator.ts)）会调用 `questionService.ask()`，但生产代码中无任何位置调用 `.reply()` 回复该实例。
...
编辑 [packages/core/src/question-service.ts](../packages/core/src/question-service.ts)，给 `ask()` 增加可选的 `requestId` 参数：
...
编辑 [packages/core/src/workflow-coordinator/coordinator.ts](../packages/core/src/workflow-coordinator/coordinator.ts)，添加公开方法：
...
编辑 [packages/tui/src/bridge.tsx](../packages/tui/src/bridge.tsx)，在 workflow event 渲染分支中添加 `ask_user` 处理：
...
编辑 [packages/tui/src/bridge.tsx](../packages/tui/src/bridge.tsx)，在 `respondQuestion()` 中添加 workflowCoordinator 转发：
...
编辑 [packages/tui/src/App.tsx](../packages/tui/src/App.tsx)，根据 `questionPrompt` state 渲染问题 UI：
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `SubagentRunner` 类（[subagent/run.ts:23](../packages/core/src/subagent/run.ts)） |
| **性质** | 死代码（与 engine.spawnSubagent 重复实现）+ 生产路径能力缺失 |
| **严重度** | 🔴 高 |

#### 现象

`SubagentRunner` 类定义了完整的 `spawnAndRun()` 方法，但生产代码从不实例化它。[engine.ts:1466](../packages/core/src/engine.ts) 自己实现了一个 `spawnSubagent()` 方法，内联了等价逻辑（创建 child engine、注册工具、应用 permission、执行 child.submit）。
...
编辑 [packages/core/src/engine.ts](../packages/core/src/engine.ts) 的 `spawnSubagent()` 方法，参考 `SubagentRunner.spawnAndRun` 的 target 解析逻辑：
...
- 编辑 [packages/core/src/subagent/index.ts](../packages/core/src/subagent/index.ts) 移除 `export { SubagentRunner } from "./run.js"`
- 编辑 [packages/core/src/index.ts:426](../packages/core/src/index.ts) 移除 `SubagentRunner` export
...
[registry.ts:55](../packages/core/src/subagent/registry.ts) 的 `defaultSubagentRegistry` 也未接入（engine 构造函数里已经是 `new SubagentRegistry()`），一并删除。
...
- 编辑 [packages/core/src/index.ts:423](../packages/core/src/index.ts) 移除 `defaultSubagentRegistry` export
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `PermissionService` 类（[permission/service.ts](../packages/core/src/permission/service.ts)） |
| **性质** | 整个模块未接入 |
| **严重度** | 🟡 中（删除属于 API 破坏） |

#### 现象

[executor-helpers.ts:84](../packages/core/src/executor-helpers.ts) 声明了可选参数 `permissionService?: PermissionService`，但生产代码从不传入此参数。engine 使用的是 `@covalo/security` 包的 `PermissionEngine`（[engine.ts:287](../packages/core/src/engine.ts) `new PermissionEngine()`）。
...
[packages/core/src/index.ts](../packages/core/src/index.ts) 仍公开导出 `PermissionService` 和相关类型。

#### 修复方案：双路径选择（根据 API 稳定性决策）

> ⚠️ **方案调整说明**：原文档建议直接删除，但 [core/index.ts](../packages/core/src/index.ts) 仍公开导出 `PermissionService`，删除属于 API 破坏。需根据 Covalo 是否有公开稳定 API 选择路径。
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `DualSession` 类、`DualSessionStore` 类（[dual-session/](../packages/core/src/dual-session/)） |
| **性质** | 整个模块未接入（被 DualAgentRuntime 替代） |
| **严重度** | 🟡 中 |

#### 现象

生产代码走 `DualAgentRuntime`（[cli/tui.ts:307](../packages/cli/src/tui.ts)），不使用 DualSession。`DualAgentRuntime` 接收 worker/supervisor engine、model target、thinking 配置。
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `inferTaskIntent`、`shouldCreateLedgerByIntent`、`hasExecutableSideSignal`（[governance/task-state.ts](../packages/core/src/governance/task-state.ts)） |
| **性质** | 整个文件未接入 |
| **严重度** | 🟢 低 |

#### 现象

生产代码 [engine.ts:1022](../packages/core/src/engine.ts) 使用的是 `task-ledger.ts` 的 `shouldCreateLedger`（不同实现），不是 `task-state.ts` 的 `shouldCreateLedgerByIntent`。
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `toWorkflowCoordinatorConfig`、`toGoalRuntimeConfig`、`getSupervisorToolPolicy`、`getWorkerToolPolicy`、`getMailboxConfig`、`getContextConfig`（[config/adapter.ts](../packages/core/src/config/adapter.ts)） |
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `onPreToolUse`、`onPreCompact`、`onSubagentStart`、`onSubagentStop`（[covalo-memory-bridge.ts:47-138](../packages/memory/src/bridge/covalo-memory-bridge.ts)） |
...
- 编辑 [packages/memory/src/bridge/covalo-memory-bridge.ts](../packages/memory/src/bridge/covalo-memory-bridge.ts)，删除 `onPreCompact`、`onSubagentStart`、`onSubagentStop` 三个方法
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `calculateCostCNY`、`getPricing`、`USD_TO_CNY`（[pricing.ts](../packages/core/src/pricing.ts)） |
| **性质** | 定义但无生产调用方 |
| **严重度** | 🟢 低 |
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `executeEccHookCommand`（[hook-bridge.ts:93](../packages/plugin/src/content-pack/hook-bridge.ts)） |
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `mergeBudgetPathSet`（[branch-budget-path.ts:51](../packages/core/src/governance/branch-budget-path.ts)） |
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `LspManager` 类（[tools/src/lsp/manager.ts](../packages/tools/src/lsp/manager.ts)） |
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `SUPERVISOR_WORKFLOW_PROMPT`（[workflow-coordinator/types.ts:203](../packages/core/src/workflow-coordinator/types.ts)） |
| **性质** | 仅 export 未使用 |
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `deriveSubagentPermissions`、`createBubbleRequest`（[subagent/permission.ts:88, 165](../packages/core/src/subagent/permission.ts)） |
| **性质** | bubble 协议未实装 |
...
编辑 [subagent/permission.ts](../packages/core/src/subagent/permission.ts)：
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `getEnabledSupervisorCandidates`（[supervisor/pool.ts:181](../packages/core/src/supervisor/pool.ts)） |
| **性质** | 仅测试使用 |
...
[pool.ts:71-72](../packages/core/src/supervisor/pool.ts) 注释明确"所有候选默认禁用，用户必须显式配置 `.covalo/supervisor-pool.json` 才能启用"。
...
[smoke.ts](../packages/core/src/supervisor/smoke.ts) 中的 `runSupervisorSmokeTest`、`runSupervisorPoolSmokeTests`、`isSupervisorSmokeEnabled` 需 `COVALO_SUPERVISOR_SMOKE=1` 才生效，生产从不调用。但它们是诊断工具，保留 export 供 CLI doctor 命令未来使用。
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `GoalRuntime` 类（[goal/runtime.ts:16](../packages/core/src/goal/runtime.ts)） |
| **性质** | 类被 import 但从未实例化 |
| **严重度** | 🟡 中 |
...
| 项 | 详情 |
|----|------|
| **未接入代码** | `useMailboxWorkflow: true` 路径（[coordinator.ts:434-519](../packages/core/src/workflow-coordinator/coordinator.ts)） |
| **性质** | 代码存在但默认禁用 |
| **严重度** | 🟡 中 |
...
本报告是对 [self_harness_audit_20260703.md](../docs/self_harness_audit_20260703.md) 的补充。self-harness 审计已覆盖 `harness-evolution/` 模块的接入状态，本报告覆盖其余所有模块。两份报告合起来构成项目完整的"未接入功能"全景。
